  ...
  while(1)
    {
      ...
      // 物体検出のプログラム
      cv::Mat L_masked = mask_background(L_trim_frame, 1500, 75);
      cv::Mat F_masked = mask_background(F_trim_frame, 1500, 95);
      //cv::Mat R_masked = mask_background(R_trim_frame, 1500, 75);
      
      // 縮小
      cv::resize(L_masked, L_trim_frame, cv::Size(), 0.41, 0.41);//197x197
      cv::resize(F_masked, F_trim_frame, cv::Size(), 0.41, 0.41);
      //cv::resize(R_masked, R_trim_frame, cv::Size(), 0.41, 0.41);
